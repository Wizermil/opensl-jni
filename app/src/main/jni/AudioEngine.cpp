...
JNIEXPORT bool JNICALL Java_com_prettysimple_audio_AudioPlayer_init(JNIEnv *env, jobject thiz,
    ...
    const int audioId = getAudioId(env,
                                   thiz); // try to recover the audioIde from the AudioPlayer.java object

    if (audioId < 0) {
        const char *pathC = env->GetStringUTFChars(path, nullptr);
        AudioPlayer *player = AudioEngine::getInstance()->createPlayerWithPath(pathC,
                                                                               (float) volume,
                                                                               (bool) loop); // Can return nullptr if the audio engine or the assemanager is not init correctly
        env->ReleaseStringUTFChars(path, pathC);

        if (player != nullptr) {
            setAudioId(env, thiz,
                       player->getPlayerId()); // Set the audioId member of java class AudioPlayer.java
            player->setJavaAudioPlayerObj(
                    thiz); // Store the jobject in GlobalRef to be able to easily set the audioId back when you stop a sound
...
/**
  ...
  * It's a convinient method to easily pause all the sounds at once
  */
JNIEXPORT bool JNICALL Java_com_prettysimple_audio_AudioEngine_resumeAll(JNIEnv *env,
                                                                         jobject thiz) {
...
AudioEngine::AudioEngine() : _audioIds(0), _engineObject(nullptr), _engineEngine(nullptr),
                             _outputMixObject(nullptr), _assetManager(nullptr), _stopGc(false),
                             _doneGc(false) {
}
...
void AudioEngine::clean() noexcept {
    ...
    if (!_doneGc &&
        _threadGc.joinable()) // Kill the thread in charge of cleaning up the list of *AudioPlayer
    {
        _stopGc = true;
...
void AudioEngine::setAssetManager(const jobject assetManager) {
    ...
    if (jenv != nullptr) {
        if (_assetManager !=
            nullptr) // Just a security to make sure that we don't create too many GlobalRef
        {
            jenv->DeleteGlobalRef(_assetManager);
            _assetManager = nullptr;
        }
 ...
 */
AudioPlayer *AudioEngine::createPlayerWithPath(const std::string &fileFullPath, const float volume,
                                               const bool loop) noexcept {
    AudioPlayer *ret = nullptr;
    if (initOpenSL() && _assetManager != nullptr) {
        ret = new AudioPlayer();
        const bool init = ret->initWithEngine(_engineEngine, _outputMixObject, getAssetManager(),
                                              ++_audioIds, fileFullPath, volume, loop);
        if (init) {
            std::lock_guard<std::mutex> lock(
                    _playersMutex); // /!\ a thread (audioPlayerGc) in charge of deleting instances of AudioEngine is running
            _players[_audioIds] = ret;
            _condition.notify_one(); // to decrease cpu usage of the thread he can be in stasis
        }
        else { // If we are not able to create the AudioPlayer we clean the memory
            delete ret;
            ret = nullptr;
        }
    }
    ...
    if (SL_RESULT_SUCCESS == result) {
        ...
        if (SL_RESULT_SUCCESS == result) {
            ...
            if (SL_RESULT_SUCCESS == result) {
                ...
                result = (*_engineEngine)->CreateOutputMix(_engineEngine, &_outputMixObject, 0,
...
bool AudioEngine::setParams(const int audioId, const float pitch, const float pan,
